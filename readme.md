...
В настоящее время происходит множество изменений в разделе Junior. Более подробную информацию вы можете найти в ветке
feature/junior. Если у вас есть свободное время и вы хотите помочь написанием текста, пожалуйста, напишите мне. Я буду
рад вашей помощи. Все материалы уже подготовлены, но у меня нет времени на написание.
...
12. Управляйте ресурсами с помощью объектов (ресурсы получаются в конструкторе и освобождаются в деструкторе.
    Рекомендуется использовать интеллектуальные указатели. Момент получения ресурса — это момент инициализации (Resource
    Acquisition Is Initialization, RAII)).
13. Будьте осторожны с поведением копирования в классах управления ресурсами (обычное поведение копирования класса RAII:
    подавление копирования, подсчет ссылок, глубокое копирование, передача прав собственности на нижний ресурс (
    аналогично auto_ptr)).

14. Предоставлять доступ к необработанным ресурсам в классе управления ресурсами (доступ к необработанным ресурсам может
...
15. Используйте ту же форму при использовании новых и удаленных в парах ( если new используется в [] середине delete []
    и new если не используется в [] середине delete).

16. Сохраните (поместите) новый объект в интеллектуальный указатель в отдельном выражении (в противном случае это может
    привести к незаметным утечкам ресурсов из-за оптимизации компилятора).

17. Сделайте интерфейс легким для правильного использования и непростым для неправильного использования (методы
...
18. Разработка класса аналогична разработке типа: необходимо учитывать создание, уничтожение, инициализацию,
    присваивание, передачу значения, юридическое значение, отношение наследования, преобразование, обобщение и т. д.

19. Скорее замените передачу по значению на передачу по ссылке на константу (первая обычно более эффективна и позволяет
    избежать проблем с нарезкой, но не работает со встроенными типами, итераторами STL, функциональными объектами).

20. Когда вы должны вернуть объект, не пытайтесь вернуть его ссылку (никогда не возвращайте указатель или ссылку,
...
21. Объявить переменные-члены как частные (для инкапсуляции, согласованности, точного контроля над их чтением и записью
    и т. д.)

22. Скорее замените функции-члены функциями, не являющимися членами и не являющимися друзьями (улучшая инкапсуляцию,
    гибкость упаковки и функциональную масштабируемость).

23. Если все параметры (включая метафорический параметр, на который указывает указатель this) требуют преобразования
    типов, используйте для этого функции, не являющиеся членами.

24. Рассмотрите возможность написания функции подкачки, которая не генерирует исключений.

25. максимально задержать появление определений переменных (увеличивает ясность программы и повышает ее эффективность)

26. Старайтесь делать как можно меньше действий преобразования (старый стиль: (T)expression, T(expression); новый стиль:
...
27. Избегайте использования дескрипторов (включая ссылки, указатели и итераторы) для указания внутри объектов (чтобы
...
28. Тщательно изучите все тонкости встраивания (встраивание — это поведение во время компиляции в большинстве программ
...
29. Минимизируйте зависимости компиляции между файлами (если вы можете использовать ссылки на объекты или указатели на
...
30. Убедитесь, что ваше общедоступное наследование формирует отношение is-a (является) (все, что относится к базовым
...
31. Избегайте затенения унаследованных имен (вы можете использовать объявления using или функции пересылки (forwarding
    functions), чтобы затененные имена снова увидели свет).
32. Различайте наследование интерфейса и наследование реализации (при общедоступном наследовании производные классы
...
33. Рассмотрите альтернативы виртуальным функциям (например, метод невиртуального интерфейса (NVI) шаблона
...
34. Никогда не переопределяйте унаследованные невиртуальные функции.

35. Никогда не переопределяйте унаследованные значения параметров по умолчанию, потому что значения параметров по
    умолчанию связаны статически (statically bound), а виртуальные функции связаны динамически (динамически связаны).

36. С помощью составления формируйте отношения has-a (имеет) или «реализуется согласно чему-то» (в прикладной области
...
37. Разумно и осторожно используйте частное наследование (частное наследование означает "реализовано-в-терминах-из" (
...
38. Разумно и осторожно используйте множественное наследование (множественное наследование сложнее, чем одиночное
...
39. Понимайте неявные интерфейсы и полиморфизм времени компиляции (классы и шаблоны поддерживают интерфейсы (interfaces)
...
40. Понимайте двойное значение typename (при объявлении параметра типа шаблона ключевые слова class и typename имеют
...
41. Изучите обработку имён в шаблонизированных базовых классах (в шаблонах производных классах можно ссылаться на члены
    шаблонных базовых классов через this-> или с помощью явного «квалификатора квалификации базового класса»).

42. Выделите код, не зависящий от параметров, из шаблонов (раздутие кода из-за параметров типа (non-type template
...
43. Используйте шаблоны функций-членов для принятия всех совместимых типов (используйте шаблоны функций-членов для
...
44. Определите нечленские функции, когда требуется преобразование типов (когда вы пишете шаблон класса, и он
...
45. Используйте классы свойств для отображения информации о типах (классы свойств с помощью шаблонов и «специализации
...
46. Познакомьтесь с метапрограммированием на шаблонах (метапрограммирование на шаблонах (TMP, template metaprogramming)
...
47. Познакомьтесь с поведением new-handler (set_new_handler позволяет клиентам указать функцию, которая будет
...
48. Познакомьтесь с правильным временем замены new и delete (чтобы обнаружить ошибки использования, собрать
...
49. При написании new и delete следуйте общепринятым правилам (operator new должен содержать бесконечный цикл и в нем
...
50. Если вы написали placement new, напишите и placement delete (когда вы пишете placement operator new, убедитесь, что
...
51. Не игнорируйте предупреждения компилятора.

52. Ознакомьтесь со стандартной библиотекой, включая TR1 (TR1, C++ Technical Report 1, черновик стандарта C++11).

53. Ознакомьтесь с Boost (почти стандартная библиотека).
