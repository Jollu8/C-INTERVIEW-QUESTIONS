...
- ### <span style="color:red">C++ OOP<span/>
  ...
  #### 70. Что такое куча и стек? Различия, принцип работы.
    ...
    - <details><summary>Ответ:</summary>
      ...
      - Куча и стек - это две области памяти, используемые программами для хранения данных. 
      - Стек - это область памяти, используемая для хранения локальных переменных и информации о вызовах функций. Он работает по принципу LIFO (последний вошел, первый вышел), то есть последняя переменная, помещенная на стек, будет первой, которая будет удалена с него. Стек имеет фиксированный размер и управляется автоматически компилятором. 
      - Куча - это область памяти, используемая для динамического выделения памяти во время выполнения программы. Она управляется программистом с помощью функций выделения и освобождения памяти, таких как `new` и `delete` в C++ или `malloc` и `free` в C. Куча имеет переменный размер и может увеличиваться или уменьшаться в зависимости от потребностей программы. 
      - ### Основные различия между кучей и стеком:
        - Стек имеет фиксированный размер и управляется автоматически, а куча имеет переменный размер и управляется вручную. 
        - Стек работает быстрее, чем куча, так как он использует простой алгоритм LIFO для управления памятью. 
        - Стек используется для хранения локальных переменных и информации о вызовах функций, а куча используется для динамического выделения памяти. 
        - Переполнение стека может привести к ошибке переполнения стека, а неправильное использование кучи может привести к утечкам памяти или повреждению данных.
