...
- ### <span style="color:red">C++ OOP<span/>
  ...
  #### 93. Как работают константные методы?
    ...
    - <details><summary>Ответ:</summary>
        ...
        - Константные методы - это методы, которые не могут изменять данные объекта, для которого они вызываются. Они
          объявляются с помощью ключевого слова `const` после списка параметров метода. Вот пример:
        - ```cpp
        class MyClass {
        public:
           MyClass(int x) : x(x) {}
        ...
        int main() {
            const MyClass myObj(10);
            std::cout << myObj.getX() << std::endl; // OK: вызов константного метода для константного объекта
            // myObj.setX(20); // Ошибка: нельзя вызвать неконстантный метод для константного объекта
        }
        ```

        - В этом примере у нас есть класс `MyClass` с константным методом `getX` и неконстантным методом `setX`. Мы
          создаем константный объект `myObj` и можем вызвать для него константный метод `getX`, но не можем вызвать
          неконстантный метод `setX`.
    </details>
